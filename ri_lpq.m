...
%LPQdesc=(G(1,:)>=0)+(G(2,:)>=0)*3+(G(3,:)>=0)*6+(G(4,:)>=0)*12+(G(5,:)>=0)*24+(G(6,:)>=0)*48+(G(7,:)>=0)*96+(G(8,:)>=0)*192;
%LPQdesc=(G(1,:)>=0)*2+(G(2,:)>=0)*4+(G(3,:)>=0)*8+(G(4,:)>=0)*16+(G(5,:)>=0)*32+(G(6,:)>=0)*64+(G(7,:)>=0)*128+(G(8,:)>=0)*256;
%% Resize and switch format to uint8 if LPQ code image is required as output
if strcmp(mode,'im')
    
    LPQdesc=uint8(LPQdesc);
   % disp('Old LPQ before reshape')
    %size(LPQdesc)
    LPQdesc=reshape(LPQdesc,[(imgRow-winSize+1),(imgCol-winSize+1)]);
    %disp('LPQ: if image needed')
   % size(LPQdesc)
end
...
%     disp('LPQ: if normalized histogram needed')
%     size(LPQdesc)
end
